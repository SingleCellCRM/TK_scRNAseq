...
# change do dir for this sample
mkdir $TAG
cd $TAG
# run cellranger
cellranger multi --id=$SAMPLE --csv=$csv

mkdir -p /exports/eddie/scratch/$USER/TK/outs/CellRanger-${TAG}/
rsync -rl $SAMPLE /exports/eddie/scratch/$USER/TK/outs/CellRanger-${TAG}/
