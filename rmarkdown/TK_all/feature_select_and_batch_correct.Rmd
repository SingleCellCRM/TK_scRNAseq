...
#library(Seurat) # batch correct CCA #moved to diferent script
```
...
```{r load-sce}
sce <- readRDS(here("processed", project, "sce_norm_newmeta_01.RDS"))
```
...
```{r savedim}
if (!(file.exists(
  here("processed", project,  "sce_dimred_01.RDS")
))) {
...
The results for the convergence plot are suspicious, similar to [these](https://github.com/immunogenomics/harmony/issues/48)
There is no implemented solution yet, so harmony is discarded. 
...
if (!(file.exists(
  ...
  harmony <- HarmonyMatrix(PCA, meta_data=colData(sce), vars_use="Chip" , do_pca=FALSE, plot_convergence = TRUE )
}
```
```{r eval=FALSE, include=FALSE}
# Attempt to see if by sorting #25 #48 would also disapear
# installed the devel version and then:
library(Seurat)
srt <- as.Seurat(sce)
harmony_srt <- RunHarmony(srt, "Chip" , plot_convergence = TRUE, kmeans_init_nstart=200, kmeans_init_iter_max=1000 , reduction = "PCA")

# but still the same convergence plot. 
```

# Seurat

Finally we test integration with [Seurat 3](https://www.cell.com/cell/fulltext/S0092-8674(19)30559-8). Also in the top 3 methods in [A benchmark of batch-effect correction methods for single-cell RNA sequencing data](https://genomebiology.biomedcentral.com/articles/10.1186/s13059-019-1850-9)

### I moved this into analyse_up_to_clustering_with_Seurat.Rmd
