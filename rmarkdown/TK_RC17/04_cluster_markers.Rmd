...
```{r}
int_res_all_mark(srt, int_cols = paste0("originalexp_snn_res.", seq(0.2, 1, 0.1)), save_dir = here("outs", project, "cluster_markers"), filter = "both")
...
# compute markers
  ...
  dir.create( here("outs", project, "cluster_markers", "ranked"))
  markers <- findMarkers(sce, groups = sce$originalexp_snn_res.1, direction="up", lfc=1)
  #saveRDS(markers, here("outs", project, "cluster_markers", "ranked.rds"))
  # save the top 100 genes for each one of the df
  lapply(names(markers), function(x){
    top_markers <- head(markers[[x]], 100)
    write.csv(top_markers, here("outs", project, "cluster_markers", "ranked", paste0("top_markers_", x, "_res.1.csv")), quote = FALSE)
    }
  )
...
# save result
write.table(top_genes, file = here("outs", project, "cluster_markers", "top_genes_nomt_res1.tsv"), sep = "\t", quote = FALSE, col.names = TRUE, row.names = FALSE)
```
