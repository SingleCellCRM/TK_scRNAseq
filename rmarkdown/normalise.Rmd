...
library(batchelor) # nromalisation per batch
```
...
between cells [(Lun et al.,
2016)](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4848819/)

The normalisation will be preformed within batches, by using the `block=batch` 
argument when performing the quick clusters, and this infromation is used to 
Perform scaling normalization within each batch to provide comparable results to the lowest-coverage batch.
This removes systematic differences in coverage across batches to simplify downstream comparisons.
...
if (!(file.exists(here("processed", project,  "sce_norm_01.RDS")))) {
  ...
  # For reproducibility
  set.seed(100)
  # Quick clustering to pool samples together and deal with 0 counts, group each chip together
  quick_clusters <- quickCluster(sce, block="Chip")
  ...
  # Perform scaling normalization within each batch to provide comparable results to the lowest-coverage batch
  sce <- multiBatchNorm(sce, batch = sce$Chip)
